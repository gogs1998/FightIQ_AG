...
def calculate_stoppage_features(df):
    ...
    # We need to reconstruct history per fighter
    # We need to know:
    # 1. Did they win?
    # 2. Was it a finish?
    # 3. Fight duration
    
    # Helper to determine if finish
    def is_finish(row):
        # Check result/method columns
        # Common values: "KO/TKO", "Submission", "Decision - Unanimous", etc.
        # We'll check multiple potential columns
        res_str = ""
    ...
    # Pre-calculate finish flags for speed
    df['is_finish'] = df.apply(is_finish, axis=1)
    
    # We need duration. If not present, assume 15 min for decision, else random?
    # Hopefully 'fight_duration_minutes' exists or 'total_time_fought(seconds)'
    if 'fight_duration_minutes' not in df.columns:
        # Try to infer from rounds/time
        # For now, fill with nan if missing
        df['duration'] = np.nan
    else:
        df['duration'] = df['fight_duration_minutes']
        
    # Build history
    history = {} # fighter_id -> {'wins': 0, 'wins_finish': 0, 'losses': 0, 'losses_finish': 0, 'total_time': 0, 'n_fights': 0}
    ...
    for idx, row in df.iterrows():
        ...
        # F1 Stats
        if h1['wins'] > 0:
            f1_finish_rate.append(h1['wins_finish'] / h1['wins'])
        else:
            f1_finish_rate.append(0.0)
            
        if h1['losses'] > 0:
            f1_been_finished_rate.append(h1['losses_finish'] / h1['losses'])
        else:
            f1_been_finished_rate.append(0.0)
            
        if h1['n_fights'] > 0:
            f1_avg_time.append(h1['total_time'] / h1['n_fights'])
        else:
            f1_avg_time.append(0.0)
            
        # F2 Stats
        if h2['wins'] > 0:
            f2_finish_rate.append(h2['wins_finish'] / h2['wins'])
        else:
            f2_finish_rate.append(0.0)
            
        if h2['losses'] > 0:
            f2_been_finished_rate.append(h2['losses_finish'] / h2['losses'])
        else:
            f2_been_finished_rate.append(0.0)
            
        if h2['n_fights'] > 0:
            f2_avg_time.append(h2['total_time'] / h2['n_fights'])
        else:
            f2_avg_time.append(0.0)
        ...
        elif winner == f2: # F1 lost
            h1['losses'] += 1
            if is_fin: h1['losses_finish'] += 1
        # Draw: counts as fight but not win/loss for this specific metric? 
        # Or maybe just ignore draws for win/loss counts.
