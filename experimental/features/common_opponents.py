...
def calculate_common_opponent_features(df):
    ...
    # We need to build a history lookup.
    # Fighter -> {Opponent: Result (1=Win, 0=Loss, 0.5=Draw)}
    # Note: We need to handle multiple fights against same opponent.
    # Let's store list of results.
    ...
    for idx, row in df.iterrows():
        ...
        if n_common > 0:
            for opp in common_opps:
                # Get results against this opponent
                res1 = hist_f1[opp] # List of 1/0
                res2 = hist_f2[opp]
                
                # Average result against this opponent
