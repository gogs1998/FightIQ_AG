...
def backtest_strategies():
    print("=== FightIQ Strategy Backtest (2024-2025) ===")
    
    # 1. Load Data & Re-Train Boruta Model (to get probs)
    ...
    model = xgb.XGBClassifier(n_estimators=500, learning_rate=0.05, max_depth=4, n_jobs=-1, random_state=42)
    ...
    for idx, row in test_df.iterrows():
        ...
        if my_prob > 0.55 and edge > 0:
            ...
            q = 1 - my_prob
            f = (b * my_prob - q) / b
            stake = bankrolls["Kelly (Quarter, Conf > 55%)"] * (f * 0.25)
            if stake < 0: stake = 0
            res = (stake * (odds - 1)) if win else -stake
            bankrolls["Kelly (Quarter, Conf > 55%)"] += res
    ...
    plt.title("FightIQ Strategy Backtest (2024-2025) - Boruta Model")
    plt.xlabel("Bets Placed")
    plt.ylabel("Bankroll ($)")
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{BASE_DIR}/experiment_2/strategy_backtest_kelly8.png')
    print("\nSaved plot to experiment_2/strategy_backtest_kelly8.png")
