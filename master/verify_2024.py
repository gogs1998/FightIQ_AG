...
def run_monte_carlo(iterations=5):
    print(f"=== Starting 2024 Verification ({iterations} runs) ===")
    ...
    train_df = df[df['event_date'] < start_date].copy()
    test_df_raw = df[(df['event_date'] >= start_date) & (df['event_date'] < end_date)].copy()
    
    # Filter for valid odds (though 2024 should be good)
    test_df = test_df_raw[(test_df_raw['f_1_odds'] > 1.0) & (test_df_raw['f_2_odds'] > 1.0)].copy()
    
    print(f"Train: {len(train_df)} (Pre-2024)")
    print(f"Test: {len(test_df)} (2024 Valid Odds) / {len(test_df_raw)} Total")
    ...
    agg_results = {k: {'roi': [], 'bankroll': []} for k in [
        "Flat (All Bets)", "Value (Edge > 5%)", "Kelly (1/8)"
    ]}
    ...
    print("\n\n=== 2024 Monte Carlo Results (5 Runs) ===")
    print(f"{'Strategy':<20} | {'Mean ROI':<10} | {'Mean Bankroll':<15} | {'Min Bankroll':<15} | {'Max Bankroll':<15}")
    print("-" * 90)
    ...
    for name, data in agg_results.items():
        ...
        print(f"{name:<20} | {mean_roi:>9.2f}% | ${mean_bk:>14.2f} | ${min_bk:>14.2f} | ${max_bk:>14.2f}")
