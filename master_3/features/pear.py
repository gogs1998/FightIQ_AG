...
def fit_pear(df_rounds: pd.DataFrame) -> pd.DataFrame:
    """
    Fit PEAR (Pace-Elasticity & Attrition Response) model for each fighter.
    Returns a DataFrame with fighter_id, beta_pace, beta_lag.
    """
    ...
    for fid, g in df.groupby('fighter_id'):
        ...
        try:
            ...
            rows.append({'fighter_id': fid,
                         'pear_beta_pace': float(beta[1]),
                         'pear_beta_lag': float(beta[2]),
                         'pear_n_rounds': int(len(g))})
        except np.linalg.LinAlgError:
            rows.append({'fighter_id': fid,
                         'pear_beta_pace': np.nan,
                         'pear_beta_lag': np.nan,
                         'pear_n_rounds': int(len(g))})
